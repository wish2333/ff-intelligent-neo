 ...
 *
 * Freeze rules:
 * - aspect_convert=none: bg_image frozen, watermark active, fullscreen drop -> watermark
 * - aspect_convert=insert: watermark frozen, bg_image active, fullscreen drop -> bg_image
 * - aspect_convert=T/B:   watermark frozen, bg_image shows info box, no fullscreen drop
 */
...
// Freeze states
const watermarkFrozen = computed(() => hasAspectConvert.value)
...
const fullscreenDropTarget = computed(() => {
  ...
  if (isInsertMode.value) return "bg_image"
  ...
  <div class="card bg-base-200 shadow-sm border border-base-300">
    <div class="card-body p-4">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <!-- Column 1: Rotate, Aspect Convert, BgImage, Target Resolution, Crop -->
        <div class="space-y-3">
          <!-- Rotate -->
          ...
          <!-- Background Image: always visible, different states -->
          <div class="form-control">
            <label class="label py-1">
              <span class="label-text text-xs">{{ t("config.filters.backgroundImage") }}</span>
            </label>
            <!-- Frozen state: no aspect_convert -->
            <FileDropInput
              v-if="!hasAspectConvert"
            ...
            />
            <label v-if="!hasAspectConvert" class="label py-0">
              <span class="label-text-alt text-xs text-base-content/50">
                {{ t("config.filters.bgImageFrozenDesc") }}
              </span>
            </label>
            <!-- Active state: insert mode -->
            <FileDropInput
              v-else-if="isInsertMode"
              :model-value="config.bg_image_path"
              :accept="fileFormats.image"
              :placeholder="t('config.filters.bgPlaceholder')"
            ...
            />
            <!-- Info state: T/B mode -->
            <div v-else class="rounded-lg border border-base-300 bg-base-300/30 px-3 py-3 text-center">
          ...
          <!-- Target Resolution: always visible -->
          <div class="form-control">
            <label class="label py-1">
          ...
          <div class="ml-4 space-y-2">
            <div class="form-control">
              ...
              <input
                ...
                max="-5"
                class="input input-bordered input-sm w-full"
                :disabled="!config.audio_normalize"
              />
            </div>
            <div class="form-control">
              <label class="label py-0">
        ...
        <div class="space-y-3">
          ...
          <!-- Watermark Image: always visible -->
          <div class="form-control">
            <label class="label py-1">
              <span class="label-text text-xs">{{ t("config.filters.watermark.image") }}</span>
            </label>
            <!-- Active state: no aspect_convert -->
            <FileDropInput
              v-if="!watermarkFrozen"
            ...
            />
            <!-- Frozen state: aspect_convert active -->
            <div v-else class="rounded-lg border border-base-300 bg-base-300/30 px-3 py-3 text-center">
              <p class="text-xs text-base-content/50">
                {{ t("config.filters.watermark.frozenDesc") }}
              </p>
            </div>
          </div>
          ...
          <div class="form-control">
            ...
            <select
              ...
              class="select select-bordered select-sm w-full"
              :disabled="!config.watermark_path"
            >
              <option
          ...
          <!-- Watermark Margin: always visible -->
          <div class="form-control">
            <label class="label py-1">
