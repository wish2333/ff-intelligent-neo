...
4. 后端测试可以使用 `uv run` 执行内联 Python 脚本
...
| ----- | ------------------------- | -------------------------------------------- | ------------------------------------------ | ---- |
| BC-01 | 基础音频编辑              | `{edit: "audio", audio_threshold: "0.04"}`   | 包含 `--edit audio`, `--my-thresh 0.04`    |      |
| BC-02 | 运动编辑                  | `{edit: "motion", motion_threshold: "0.02"}` | 包含 `--edit motion`                       |      |
...
print("BC-01:", "--edit audio" in cmd, "--my-thresh 0.04" in cmd, "--progress machine" in cmd)
...
| ID    | 测试用例   | 步骤               | 预期结果                                            | 结果 |
| ----- | ---------- | ------------------ | --------------------------------------------------- | ---- |
| FN-01 | 路由存在   | 导航到 `/auto-cut` | AutoCutPage 正常加载，无错误                        |      |
| FN-02 | 导航项可见 | 检查 AppNavbar     | "Auto Cut" 导航项可见于 AudioSubtitle 和 Merge 之间 |      |
| FN-03 | i18n 英语  | 切换至英语         | 导航显示 "Auto Cut"                                 |      |
| FN-04 | i18n 中文  | 切换至中文         | 导航显示 "自动剪辑"                                 |      |
| FN-05 | 404 重定向 | 导航到无效路由     | 重定向到 404 或首页                                 |      |
...
| ID    | 测试用例        | 步骤                    | 预期结果                                             | 结果 |
| ----- | --------------- | ----------------------- | ---------------------------------------------------- | ---- |
| FP-01 | 页面加载        | 导航到 Auto Cut         | 所有部分均渲染：状态栏、文件输入、选项卡、预览、按钮 |      |
| FP-02 | 状态栏 - 未配置 | 未设置 auto-editor 路径 | 显示 "Set auto-editor path in Settings"              |      |
| FP-03 | 状态栏 - 兼容   | 已设置有效路径          | 状态栏隐藏或显示绿色指示器                           |      |
| FP-04 | 状态栏 - 不兼容 | 版本不兼容              | 显示版本警告                                         |      |
| FP-05 | 按钮禁用        | 未配置路径              | "Add to Queue" 按钮被禁用                            |      |
| FP-06 | 按钮启用        | 已配置路径              | "Add to Queue" 按钮可点击                            |      |
...
| 5    | 检查命令预览                            | 显示 `--edit audio --my-thresh 0.04 --progress machine` |      |
