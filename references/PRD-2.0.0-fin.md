...
- [x] 通过文件对话框添加 5 个文件，全部显示为"待执行"
- [x] 通过拖拽添加文件
- [x] 选中多个任务后点击"移除选中"，任务被删除
- [x] 点击单个任务"开始"，任务转为"执行中"，进度条更新
- [x] 点击任务"停止"，任务转为"已取消"
- [x] 全部任务完成后显示正确的队列摘要
- [x] 某个任务失败时显示错误信息，其他任务继续
- [x] 选中任务后展开日志面板，显示 FFmpeg stderr 输出

### Phase 3 测试

- [x] 切换视频编码器，命令预览实时更新
- [x] 设置分辨率 1280x720，预览中出现 `scale=1280:720`
- [x] 设置音量 1.5，预览中出现 `volume=1.5`
- [x] 组合裁剪+旋转+水印，预览中 `filter_complex` 链正确
- [x] 视频编码器设为 copy 但设置了滤镜，显示验证警告
- [x] 分辨率格式错误，显示验证错误
- [x] 保存预设 -> 下拉列表中出现
- [x] 加载预设 -> 表单自动填充
- [x] 删除预设 -> 从列表移除
- [x] 使用当前配置"添加到队列"，文件出现在任务队列页面（已修改为FFmpeg配置界面设置好的设置即为全局命令配置）

### Phase 4 测试

- [x] 暂停执行中的任务 -> 进程暂停，状态显示"已暂停"
- [x] 恢复已暂停任务 -> 进程继续，进度继续更新
- [x] 停止已暂停任务 -> 状态显示"已取消"
- [x] 重试失败任务 -> 状态重置为"待执行"，可重新执行
- [x] 上移/下移任务 -> 队列顺序变更
- [x] 全部暂停/恢复/停止 -> 批量操作正确执行
- [x] 修改线程数 -> 下次任务调度使用新值
- [x] 设置自定义输出文件夹 -> 任务使用该文件夹
