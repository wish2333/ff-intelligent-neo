...
所有模型使用 `@dataclass(frozen=True)` 不可变数据类。
...
TaskConfig              # 单个任务的完整配置
  transcode: TranscodeConfig
  filters: FilterConfig
  output_dir: str

TaskProgress            # 任务进度（实时更新）
...
Task                    # 任务实体
  ...
  output_path, error, log_lines, created_at, started_at, completed_at
...
| 方法 | 参数 | 返回 | 说明 |
|------|------|------|------|
| `add_tasks` | paths: list[str], config: dict | tasks: list[Task] | 探测文件并添加到队列 |
| `remove_tasks` | task_ids: list[str] | None | 删除指定任务 |
| `reorder_tasks` | task_ids: list[str] | None | 按给定ID顺序重排队列 |
| `get_tasks` | - | tasks: list[Task] | 获取所有任务（队列顺序） |
| `get_queue_summary` | - | summary: dict | 获取各状态任务计数 |
| `clear_completed` | - | removed_count: int | 清除已完成任务 |
| `clear_all` | - | None | 清空所有任务 |

#### 任务控制操作

| 方法 | 参数 | 返回 | 说明 |
|------|------|------|------|
| `start_task` | task_id: str | None | 开始执行单个任务 |
| `pause_task` | task_id: str | None | 暂停单个任务 |
| `resume_task` | task_id: str | None | 恢复单个任务 |
| `stop_task` | task_id: str | None | 停止单个任务 |
| `retry_task` | task_id: str | None | 重试失败任务 |
| `pause_all` | - | None | 暂停所有运行中任务 |
| `stop_all` | - | None | 停止所有非终态任务 |
| `resume_all` | - | None | 恢复所有已暂停任务 |

#### 配置与预设操作

| 方法 | 参数 | 返回 | 说明 |
|------|------|------|------|
| `build_command` | config: dict | command: str | 预览FFmpeg命令 |
| `validate_config` | config: dict | errors: list, warnings: list | 验证配置参数 |
| `get_presets` | - | presets: list[Preset] | 获取所有预设 |
| `save_preset` | preset: dict | preset: Preset | 保存预设 |
| `delete_preset` | preset_id: str | None | 删除预设 |

#### 设置与系统操作

| 方法 | 参数 | 返回 | 说明 |
|------|------|------|------|
| `get_settings` | - | settings: AppSettings | 获取应用设置 |
| `save_settings` | settings: dict | None | 保存应用设置 |
| `setup_ffmpeg` | - | info: dict | 自动检测并初始化FFmpeg |
| `switch_ffmpeg` | path: str | info: dict | 切换到指定路径的FFmpeg |
| `select_ffmpeg_binary` | - | path: str | 打开文件对话框选择ffmpeg.exe |
| `get_ffmpeg_versions` | - | versions: list | 返回所有已知FFmpeg版本列表 |
| `download_ffmpeg` | - | path: str | 下载预编译FFmpeg |
| `get_app_info` | - | info: dict | 获取应用和FFmpeg版本信息 |
| `select_files` | - | paths: list[str] | 打开文件选择对话框 |
| `select_output_dir` | - | dir: str | 打开文件夹选择对话框 |
...
当有多个滤镜时，合并为 `-filter_complex` 表达式：
...
1. crop (裁剪)
2. scale (缩放，来自分辨率配置)
3. rotate (旋转，使用 transpose)
4. speed (速度，使用 setpts + atempo)
5. overlay (水印叠加)
...
| 序号 | 内容 | 类型 |
|------|------|------|
| 1 | `core/models.py` 全部数据模型 | 新建 |
| 2 | `core/config.py` AppSettings 加载/保存 | 新建 |
| 3 | `core/task_queue.py` 任务CRUD + 状态机 + 持久化 | 新建 |
| 4 | `frontend/src/router.ts` vue-router 配置 | 新建 |
| 5 | `frontend/src/main.ts` 挂载 router | 修改 |
| 6 | `frontend/src/App.vue` Navbar + router-view | 重写 |
...
| 11 | 安装 vue-router 依赖 | 配置 |

### Phase 2: 任务队列后端 + 基本UI

**目标**: 可添加文件、在列表中查看、启动/停止队列、查看进度

| 序号 | 内容 | 类型 |
|------|------|------|
| 1 | `core/task_runner.py` 线程池 + 任务分发 + 停止 | 新建 |
| 2 | `core/ffmpeg_runner.py` 重构（每任务cancel_event） | 重写 |
| 3 | `core/command_builder.py` 基础转码命令生成 | 新建 |
| 4 | `main.py` 任务相关 @expose 方法 | 重写 |
| 5 | 全部 task-queue 组件 (7个) | 新建 |
| 6 | useTaskQueue, useTaskControl, useTaskProgress | 新建 |
| 7 | useFileDrop（从1.x迁移） | 迁移 |
| 8 | TaskQueuePage.vue 完整实现 | 重写 |
| 9 | `frontend/src/utils/format.ts` | 新建 |
...
| 序号 | 内容 | 类型 |
|------|------|------|
| 1 | `core/command_builder.py` 添加滤镜链支持 | 扩展 |
| 2 | `core/preset_manager.py` 重写 | 重写 |
| 3 | `main.py` 配置/预设 @expose 方法 | 扩展 |
| 4 | 全部 config 组件 (6个) | 新建 |
| 5 | useConfig, usePresets, useCommandPreview | 新建 |
| 6 | CommandConfigPage.vue 完整实现 | 重写 |
...
| 序号 | 内容 | 类型 |
|------|------|------|
| 1 | `core/task_runner.py` 暂停/恢复（进程挂起） | 扩展 |
| 2 | `main.py` 暂停/恢复/重试/设置方法 | 扩展 |
| 3 | 全部 settings 组件 (3个) | 新建 |
...
```
  ...
  1.1 数据模型 (PRD 6.1 vs core/models.py)

  ┌─────────────────┬──────────────────────┬────────────────────────────────────────────────────────────────────────┐
  │    PRD 模型     │       现有对应       │                                  差距                                  │
  ├─────────────────┼──────────────────────┼────────────────────────────────────────────────────────────────────────┤
  │ TaskState       │ TaskProgress.status  │ 需完全重写，现有用宽松字符串                                           │
  │                 │ (string, 非Literal)  │                                                                        │
  ├─────────────────┼──────────────────────┼────────────────────────────────────────────────────────────────────────┤
  │ TranscodeConfig │ 不存在               │ 新建                                                                   │
  ├─────────────────┼──────────────────────┼────────────────────────────────────────────────────────────────────────┤
  │ FilterConfig    │ 不存在               │ 新建                                                                   │
  ├─────────────────┼──────────────────────┼────────────────────────────────────────────────────────────────────────┤
  │ TaskConfig      │ 不存在               │ 新建                                                                   │
  ├─────────────────┼──────────────────────┼────────────────────────────────────────────────────────────────────────┤
  │ TaskProgress    │ TaskProgress         │ 字段差异大：现有用 file_index/time_str/status，PRD 用                  │
  │                 │ (L36-49)             │ percent/current_seconds/total_seconds/fps/frame/estimated_remaining    │
  ├─────────────────┼──────────────────────┼────────────────────────────────────────────────────────────────────────┤
  │ Task            │ FileItem +           │ 合并了两个现有类，需新建                                               │
  │                 │ TaskProgress         │                                                                        │
  ├─────────────────┼──────────────────────┼────────────────────────────────────────────────────────────────────────┤
  │ Preset          │ Preset (L23-32)      │ PRD 的 Preset 包含 config: TaskConfig，现有是 command_template 字符串  │
  │                 │                      │ -- 结构完全不同                                                        │
  ├─────────────────┼──────────────────────┼────────────────────────────────────────────────────────────────────────┤
  │ AppSettings     │ 不存在               │ 新建                                                                   │
  └─────────────────┴──────────────────────┴────────────────────────────────────────────────────────────────────────┘

  结论: core/models.py 需完全重写。但现有 FileItem 的 probe_file() 返回值可复用于 Task 的元信息填充。

  1.2 预设结构 (PRD vs default_presets.json)

  重大差异: 现有预设用 command_template: "-i {input} -vn -c:a libmp3lame -b:a 320k {output}"，PRD 要求结构化为 config: {
   transcode: TranscodeConfig, filters: FilterConfig }。这意味着所有 4 个内置预设需重写，PresetManager 也需重写。

  1.3 Bridge API (PRD 6.3 vs main.py)

  ┌─────────────────────────────┬─────────────────────────────────┬────────────────────────────────────────┐
  │          现有方法           │            PRD 对应             │                  变化                  │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ add_files(paths)            │ add_tasks(paths, config)        │ 参数变了，行为变了（需要 config 参数） │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ remove_files(indices)       │ remove_tasks(task_ids)          │ 按 ID 删除，不再按索引                 │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ clear_files()               │ clear_all()                     │ 语义不变                               │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ get_files()                 │ get_tasks()                     │ 返回 Task 而非 FileItem                │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ start_batch(preset_id, ...) │ start_task(task_id)             │ 从"批量一键"变为"逐任务启动"           │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ cancel_batch()              │ stop_task(task_id) + stop_all() │ 粒度变细                               │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ get_presets()               │ get_presets()                   │ 预设结构变了                           │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ save_preset(data)           │ save_preset(preset)             │ 预设结构变了                           │
  ├─────────────────────────────┼─────────────────────────────────┼────────────────────────────────────────┤
  │ delete_preset(id)           │ delete_preset(id)               │ 基本相同                               │
  └─────────────────────────────┴─────────────────────────────────┴────────────────────────────────────────┘

  PRD 新增 18 个 @expose 方法，现有仅 12 个。main.py 需大幅扩展。
  ...
  │    项目     │        现有         │                    PRD 要求                     │
  ├─────────────┼─────────────────────┼─────────────────────────────────────────────────┤
  │ 路由        │ 无 (单页面)         │ vue-router hash 模式，3 个页面                  │
  ├─────────────┼─────────────────────┼─────────────────────────────────────────────────┤
  │ 组件        │ 10 个 (扁平结构)    │ 20+ 个 (layout/task-queue/config/settings 分组) │
  ├─────────────┼─────────────────────┼─────────────────────────────────────────────────┤
  │ Composables │ 4 个                │ 9 个                                            │
  ├─────────────┼─────────────────────┼─────────────────────────────────────────────────┤
  │ 类型定义    │ env.d.ts 中         │ 独立 types/ 目录，4 个文件                      │
  ├─────────────┼─────────────────────┼─────────────────────────────────────────────────┤
  │ 状态管理    │ composable 本地状态 │ provide/inject 全局共享                         │
  └─────────────┴─────────────────────┴─────────────────────────────────────────────────┘

  vue-router 未安装，package.json 无此依赖。

  1.5 缺失的依赖

  - 前端: vue-router (PRD 明确要求)
  - 前端: daisyui v5 和 tailwindcss v4 已满足 (package.json 已有)

  ---
  二、PRD 内容错误

  2.1 Section 9.4 验证规则 -- 内容错误 (严重)

  L945-962: Section 9.4 的内容与 Section 9.3 (L912-931) 完全重复，都是命令示例，没有实际的验证规则内容。

  9.4 标题是"验证规则"，但内容是 ffmpeg 命令示例（和 9.3 一模一样），这是明显的 copy-paste 错误。4.5
  节定义了验证规则（L322-332），但 9.4 节没有实现层面的验证规则描述。

  2.2 Section 9.2 滤镜排序 -- 自相矛盾

  PRD L905-910 定义了"固定顺序":

  ▎ crop -> scale -> rotate -> speed -> overlay

  但 L880-887 的 priority 表是:

  ▎ crop(5) -> denoise(10) -> scale(20) -> rotate(30) -> speed(40) -> overlay(50)

  这两者说法不一致：一处说是"固定顺序"，另一处说是"优先级自动排序"。实际实现只会是一种，需要统一措辞。

  2.3 Section 5.2 FFmpeg 版本切换 -- 1.x 中不存在

  PRD L403-425 定义了 5 个 FFmpeg 版本管理 API (switch_ffmpeg, select_ffmpeg_binary, get_ffmpeg_versions,
  download_ffmpeg)，但现有 core/ffmpeg_setup.py 仅有 ensure_ffmpeg() 和 get_ffmpeg_path() 两个基础函数。这些 API 在
  Phase 4 才实现 (PRD L1066-1068)，但 5.2 节和 6.3 节已经定义了完整接口 -- 这本身没问题，但 PRD 没有标注哪些 API
  是新增、哪些是从 1.x 保留的，容易造成混淆。

  2.4 Section 6.1 frozen dataclass 与可变状态矛盾

  PRD L459 声明"所有模型使用 @dataclass(frozen=True) 不可变数据类"，但 Task 模型包含 state、progress、log_lines
  这些高频变化的字段。frozen dataclass 意味着每次状态变更都需要创建新的 Task 副本（dataclasses.replace(task,
  state=new_state)），这在任务队列中会导致大量对象创建。

  建议改为: Task 的配置部分用 frozen dataclass，Task 本身用普通 class 或 dataclass (不 frozen)。
  ...
  │          问题          │     位置     │                                说明                                │
  ├────────────────────────┼──────────────┼────────────────────────────────────────────────────────────────────┤
  │ vue-router 安装位置    │ Phase 1, #11 │ 应在 #4 (router.ts) 之前，且应在 #5 (main.ts) 之前                 │
  ├────────────────────────┼──────────────┼────────────────────────────────────────────────────────────────────┤
  │ models.py 是全后端基础 │ Phase 1, #1  │ 正确，但它依赖什么？没有前置依赖                                   │
  ├────────────────────────┼──────────────┼────────────────────────────────────────────────────────────────────┤
  │ 缺少 Bridge 层重构     │ Phase 2, #4  │ main.py 的 Bridge API 变化太大 (18 个新方法)，应该拆分到多个 phase │
  └────────────────────────┴──────────────┴────────────────────────────────────────────────────────────────────┘
  ...
  │  Phase  │ 项数  │                                   工作量评估                                   │
  ├─────────┼───────┼────────────────────────────────────────────────────────────────────────────────┤
  │ Phase 1 │ 11 项 │ 偏轻 -- 但新建 router + 全部 types + 重写 App.vue 已不小                       │
  ├─────────┼───────┼────────────────────────────────────────────────────────────────────────────────┤
  │ Phase 2 │ 9 项  │ 最重 -- 后端 task_runner + ffmpeg_runner 重写 + 7 个前端组件 + 3 个 composable │
  ├─────────┼───────┼────────────────────────────────────────────────────────────────────────────────┤
  │ Phase 3 │ 6 项  │ 中等 -- command_builder 扩展 + 6 个组件                                        │
  ├─────────┼───────┼────────────────────────────────────────────────────────────────────────────────┤
  │ Phase 4 │ 6 项  │ 中等 -- 进程挂起是最复杂的技术点，仅列为 1 项                                  │
  ├─────────┼───────┼────────────────────────────────────────────────────────────────────────────────┤
  │ Phase 5 │ 5 项  │ 偏轻 -- 持久化 + UI 打磨                                                       │
  └─────────┴───────┴────────────────────────────────────────────────────────────────────────────────┘

  建议: Phase 2 过重，建议拆为 2a (后端 task_runner + ffmpeg_runner) 和 2b (前端 task-queue 组件)。
  ...
  │                         │   Phase   │                                                                           │
  ├─────────────────────────┼───────────┼───────────────────────────────────────────────────────────────────────────┤
  │ Bridge 层重构策略       │ Phase 1   │ main.py 从 12 方法扩到 30+ 方法，需要模块化拆分 (e.g. TaskApiMixin,       │
  │                         │           │ ConfigApiMixin)                                                           │
  ├─────────────────────────┼───────────┼───────────────────────────────────────────────────────────────────────────┤
  │ bridge.ts 更新          │ Phase 1   │ 现有 bridge.ts 的 call() 接口需适配新的统一返回格式                       │
  ├─────────────────────────┼───────────┼───────────────────────────────────────────────────────────────────────────┤
  │ 现有代码清理            │ Phase 1   │ batch_runner.py、现有 main.py 中的旧方法需要在某阶段明确废弃              │
  ├─────────────────────────┼───────────┼───────────────────────────────────────────────────────────────────────────┤
  │ FFmpeg                  │ Phase 2   │ 标注"新建"，但缺少从 1.x 的 command_template 到结构化参数的迁移路径       │
  │ 命令构建器基础转码      │ #3        │                                                                           │
  ├─────────────────────────┼───────────┼───────────────────────────────────────────────────────────────────────────┤
  │ 内置预设 JSON 更新      │ Phase 5   │ 4 个预设从 command_template 格式变为 config 格式，但工作量被低估          │
  │                         │ #4        │                                                                           │
  └─────────────────────────┴───────────┴───────────────────────────────────────────────────────────────────────────┘

  3.4 风险项错位

  PRD L1062: Phase 4 第 1 项 "core/task_runner.py 暂停/恢复（进程挂起）"，这实际上是整个 2.0 中技术风险最高的部分:
  - SuspendProcess 不是公开 API，文档稀缺
  - stderr 管道缓冲区满导致死锁
  - 进程权限问题
  - Linux/macOS 需要完全不同的实现

  应在此项前增加"技术验证"步骤（单独跑一个最小 demo 验证 SuspendProcess 可行性），而非直接列入 Phase 4 实施。

  ---
  四、风险评估补充

  PRD Section 12 缺少以下风险:

  ┌────────────────────────────┬────────┬──────────────────────────────────────────────────────────────┐
  │            风险            │ 严重度 │                             说明                             │
  ├────────────────────────────┼────────┼──────────────────────────────────────────────────────────────┤
  │ frozen Task 不可变更新性能 │ 中     │ 高频状态更新时大量 replace() 调用                            │
  ├────────────────────────────┼────────┼──────────────────────────────────────────────────────────────┤
  │ 命令注入                   │ 高     │ 用户通过滤镜参数（crop、watermark_path）注入恶意 FFmpeg 参数 │
  ├────────────────────────────┼────────┼──────────────────────────────────────────────────────────────┤
  │ 磁盘空间耗尽               │ 中     │ 批量转码大文件时无空间检查，导致静默失败                     │
  ├────────────────────────────┼────────┼──────────────────────────────────────────────────────────────┤
  │ 多实例并发                 │ 低     │ 两个实例同时写 queue_state.json 导致数据损坏                 │
  ├────────────────────────────┼────────┼──────────────────────────────────────────────────────────────┤
  │ FFmpeg 版本兼容性          │ 中     │ 某些滤镜参数在不同 FFmpeg 版本间语法不同                     │
  └────────────────────────────┴────────┴──────────────────────────────────────────────────────────────┘
  ...
  │      维度      │ 评分 │                         说明                         │
  ├────────────────┼──────┼──────────────────────────────────────────────────────┤
  │ 完整性         │ 7/10 │ 覆盖面广，但 9.4 内容完全错误，部分 API 定义缺少细节 │
  ├────────────────┼──────┼──────────────────────────────────────────────────────┤
  │ 一致性         │ 6/10 │ frozen dataclass 矛盾、滤镜排序矛盾、9.3/9.4 重复    │
  ├────────────────┼──────┼──────────────────────────────────────────────────────┤
  │ 可行性         │ 8/10 │ 大部分方案可行，进程挂起方案需要先做技术验证         │
  ├────────────────┼──────┼──────────────────────────────────────────────────────┤
  │ 计划颗粒度     │ 6/10 │ Phase 2 过重，Phase 4 风险项过轻，缺少代码清理阶段   │
  ├────────────────┼──────┼──────────────────────────────────────────────────────┤
  │ 与现有代码差距 │ -    │ 需重写 ~80% 后端、~90% 前端，工作量较大              │
  └────────────────┴──────┴──────────────────────────────────────────────────────┘

  ---
  六、建议修改项 (按优先级)

  1. [必须修复] 9.4 节内容错误 -- 补充实际的验证规则实现说明
  2. [必须修复] 9.2 滤镜排序矛盾 -- 统一为"基于 priority 自动排序"
  3. [建议修改] 6.1 Task 模型 -- 改为 mutable，配置子对象保持 frozen
  4. [建议修改] Phase 2 拆分 -- 分为后端和前端两个子阶段
  5. [建议修改] Phase 4 前增加技术验证 -- SuspendProcess 最小 demo
  6. [建议补充] Section 12 补充命令注入风险
  7. [建议补充] Phase 1 增加 bridge.ts 更新和现有代码清理
  8. [建议补充] 标注 Bridge API 中哪些从 1.x 保留、哪些新增
```
